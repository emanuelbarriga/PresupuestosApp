# Archivador Mensual Specification

> Capability: `archivador-mensual` · Date: 2026-07-14 · Source: Change `organizacion-mensual`

## Purpose

Provide a monthly accounting filing cabinet view that lets accountants review all classified documents (`status == "enlazado"`) grouped by the 8 accounting categories, facilitating month-end closure. Accessible as a second tab within `/media`.
...
When "Sin periodo" is selected, the query switches to `where('periodo', '==', 'sin_periodo')`. A visual indicator (e.g., badge or chip color change) SHALL make it clear that the unclassified filter is active. The year selector SHALL be disabled when "Sin periodo" is selected, showing "—" to avoid the confusing state "year 2026 + sin periodo".
...
- AND the year select is disabled showing "—"
...
- AND the year select re-enables
...
The system SHALL display the sum of `montoTotal` for the visible category using `Number()` + `isNaN()` guard with `Math.round()` before formatting. An indicator SHALL show "X de Y documentos con monto" where X is the count of documents with a valid numeric `montoTotal` and Y is the total documents in the category.
...
The system SHALL show a banner at the top of the Archivador when documents with `status: 'enlazado'` and `periodo == 'sin_periodo'` exist. The count SHALL be fetched with `getCountFromServer()` on mount (not `onSnapshot`). Clicking the banner SHALL switch the year-month selector to `'sin_periodo'`.
...
### Requirement: Index-Building Error Handling

When the required composite index (`periodo ASC, status ASC`) is still being built, the system SHALL capture the Firestore `failed-precondition` error and display a friendly message: "Estamos preparando tu archivador por primera vez, esto tomará un par de minutos..." with a retry button.

#### Scenario: Index building in progress

- GIVEN the composite index for `periodo + status` has not finished building
- WHEN ArchivadorTab mounts and tries the query
- THEN the error `failed-precondition` is captured
- AND a friendly loading state is shown with retry button
...
### Requirement: Schema Defaults for periodo and tipoDocumento

When creating or updating a `DocumentoMedio`, if `periodo` is not provided, the system SHALL default to `'sin_periodo'`. If `tipoDocumento` is not provided, the system SHALL default to `'otro'`. This applies at the schema/save layer (`lib/schemas.ts`), not at the form validation level.
...
### Requirement: Firestore Rules for Enlazado Validation
