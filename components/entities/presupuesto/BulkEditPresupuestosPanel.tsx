...
import { batchUpdatePresupuestos, getTerceros, subscribeProjects } from '@/lib/firestore';
...
export function BulkEditPresupuestosPanel({
  ...
  const buildPayload = (): Record<string, any> => {
    ...
    if (montoPresupuestado !== '') {
      data.montoPresupuestado = Number(montoPresupuestado);
    }
    if (projectId !== '') {
  ...
  return (
    <div className="flex flex-col h-full w-[360px] absolute inset-0">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5 select-text">
        ...
        <div>
          <label className="block text-[10px] font-bold text-slate-500 uppercase mb-2">Monto Presupuestado</label>
          <input type="text" inputMode="numeric"
            value={montoEditing ? unformatThousands(montoPresupuestado) : formatThousands(montoPresupuestado)}
            onFocus={() => setMontoEditing(true)}
            onBlur={() => setMontoEditing(false)}
            onChange={e => setMontoPresupuestado(unformatThousands(e.target.value))}
