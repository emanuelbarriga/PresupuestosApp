...
interface ExtractoAddFormProps {
...
/**
 * "Add extracto" flow: drag & drop zone. Uses `parseForPreview` for
 * non-persisting PDF parsing, then opens `ExtractoParseModal` for review.
 * Persistence happens via `onSubmit` callback.
 */
export function ExtractoAddForm({
  ...
  /**
   * Core parse step: extract text from the buffer, detect bank,
   * then call `parseForPreview` for full processing.
   */
  const runParse = async (selected: File, bancoForzado?: Banco | null) => {
    ...
    try {
      ...
      if (bancoDetectado === 'No detectado') {
        ...
        setHeader({ mes: '', anio: new Date().getFullYear(), banco: 'No detectado', saldoInicial: 0, saldoFinal: 0 });
        setMovimientos([]);
        setLoading(false);
        return;
      }
