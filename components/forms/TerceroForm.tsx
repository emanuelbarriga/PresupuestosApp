...
interface TerceroFormProps {
  form: { mode: 'add' | 'edit'; type: 'client' | 'provider' | 'tercero'; record?: any; defaults?: Record<string, string> };
  title: string;
...
export function TerceroForm({
  ...
  const [fields, setFields] = useState<TerceroFormData>(() => {
    ...
    return {
      ...
      lugar: defs.lugar ?? '',
      tipo: defs.tipo ?? (form.type === 'client' ? 'cliente' : form.type === 'provider' ? 'proveedor' : 'cliente'),
    };
  });
